...
import com.runescape.animations.Animation;
import com.runescape.animations.Frame;
import com.runescape.animations.Graphic;
...
import com.runescape.cache.utils.FileStore;
import com.runescape.cache.utils.FileStore.Store;
import com.runescape.collection.Deque;
import com.runescape.collection.Linkable;
import com.runescape.configuration.Constants;
...
public class Client extends GameApplet {
	...
	public void savePlayerData() {
		try {
			...
			if (stream != null) {
				...
				stream.writeBoolean(Constants.alwaysLeftClickAttack);
				stream.writeBoolean(Constants.combatOverlayBox);
				stream.writeBoolean(Constants.hitmarks554);
				stream.writeBoolean(Constants.hpBar554);
				stream.writeBoolean(Constants.enableTweening);
				stream.writeBoolean(Constants.enableRoofs);
				stream.writeBoolean(Constants.enableFog);
				stream.writeBoolean(Constants.bountyHunterInterface);
				stream.writeBoolean(Constants.escapeCloseInterface);
	...
	private void loadPlayerData() throws IOException {
		...
		try {
			...
			Constants.alwaysLeftClickAttack = stream.readBoolean();
			Constants.combatOverlayBox = stream.readBoolean();
			Constants.hitmarks554 = stream.readBoolean();
			Constants.hpBar554 = stream.readBoolean();
			Constants.enableTweening = stream.readBoolean();
			Constants.enableRoofs = stream.readBoolean();
			Constants.enableFog = stream.readBoolean();
			Constants.bountyHunterInterface = stream.readBoolean();
			Constants.escapeCloseInterface = stream.readBoolean();
	...
	private void updateSettings() {
		settings[809] = Constants.alwaysLeftClickAttack ? 1 : 0;
		settings[810] = Constants.combatOverlayBox ? 1 : 0;
		settings[811] = Constants.hitmarks554 ? 1 : 0;
		settings[812] = Constants.hpBar554 ? 1 : 0;
		settings[813] = Constants.enableTweening ? 1 : 0;
		settings[814] = Constants.enableRoofs ? 1 : 0;
		settings[815] = Constants.enableFog ? 1 : 0;
		settings[816] = Constants.bountyHunterInterface ? 1 : 0;
		settings[817] = Constants.escapeCloseInterface ? 1 : 0;
	}
	...
	private void drawChatArea() {
		...
		if (messagePromptRaised) {
			...
			newRegularFont.drawBasicString(inputString + (tick % 40 < 20 ? "|" : ""), xOffset + 12 + font.getTextWidth(s + ": "), 133 + y, changeChatArea ? 0x7FA9FF : 255, shadow);
	...
	public void updateVarp(int id) {
		...
		if (parameter == 3) {
			...
			boolean previousPlayingMusic = Constants.enableMusic;
			...
			if (Constants.enableMusic != previousPlayingMusic && !lowMemory) {
					...
					if (player.headIcon >= 0) {
						...
						if (spriteDrawX > -1) {
							if (player.skullIcon < 2) {
								...
								l += 25;
								if (Constants.hpAboveHeads && Constants.namesAboveHeads) {
									text_over_head_offset -= 25;
								} else if (Constants.namesAboveHeads) {
									text_over_head_offset -= 23;
								} else if(Constants.hpAboveHeads) {
			...
			for (int i = 0; i < sideIconsTab.length; i++) {
				if (tabInterfaceIDs[sideIconsTab[i]] != -1) {
					if (sideIconsId[i] != -1) {
						...
						if(i == 2) {
							Client.cacheSprite[491].drawAdvancedSprite(sideIconsX[i] + xOffset,
									sideIconsY[i] + yOffset);
						} else {
	...
	private void updateNPCMovement(int i, Buffer stream) {
		while (stream.bitPosition + 21 < i * 8) {
			...
			npc.desc = NpcDefinition.lookup(stream.readBits(Constants.npcBits));
	...
	private void setupLoginScreen() {
		...
		if (titleArchive != null) {
			cacheSprite[494].drawAdvancedSprite(0, 0);
			loadTitleScreen();
		}
		welcomeScreenRaised = true;
	}
	...
	public void drawLoadingText(int i, String s) {
		...
		char c1 = '\310';
		byte byte1 = 20;
		boldText.drawText(0xffffff, Constants.getClientName() + " is loading - please wait...",
				c1 / 2 - 26 - byte1, c / 2);
		int j = c1 / 2 - 18 - byte1;
	...
	public void playSong(int id) {
		if (id != currentSong && Constants.enableMusic && !lowMemory && prevSong == 0) {
			nextSong = id;
			fadeMusic = true;
			resourceProvider.provide(2, nextSong);
			currentSong = id;
	...
	private void processMenuActions(int id) {
		...
		// custom
		if (action == 1506 && Constants.enableOrbs) { // Select quick
			// prayers
		...
		// custom
		if (action == 1500 && Constants.enableOrbs) { // Toggle quick
			// prayers
		...
		// custom
		if (action == 1508 && Constants.enableOrbs) { // Toggle HP above
			// heads
			Constants.hpAboveHeads = !Constants.hpAboveHeads;
		}
			...
			if (flag8) {
				...
				switch (button) {
					...
					case 31011:
						...
						break;
					case 23003:
						Constants.alwaysLeftClickAttack = !Constants.alwaysLeftClickAttack;
						savePlayerData();
						updateSettings();
						break;
					case 23005:
						Constants.combatOverlayBox = !Constants.combatOverlayBox;
						savePlayerData();
						updateSettings();
						break;
					case 23007:
						Constants.hitmarks554 = !Constants.hitmarks554;
						savePlayerData();
						updateSettings();
						break;
					case 23009:
						Constants.hpBar554 = !Constants.hpBar554;
						savePlayerData();
						updateSettings();
						break;
					case 23011:
						Constants.enableTweening = !Constants.enableTweening;
						savePlayerData();
						updateSettings();
						break;
					case 23013:
						Constants.enableRoofs = !Constants.enableRoofs;
						savePlayerData();
						updateSettings();
						break;
					case 23015:
						Constants.enableFog = !Constants.enableFog;
						savePlayerData();
						updateSettings();
						break;
					case 23024:
						Constants.bountyHunterInterface = !Constants.bountyHunterInterface;
						savePlayerData();
						updateSettings();
						break;
					case 53007:
						Constants.escapeCloseInterface = !Constants.escapeCloseInterface;
						savePlayerData();
						updateSettings();
						break;
					case 53002:
	...
	private void login(String name, String password, boolean reconnecting) {
		...
		try {
			...
			socketStream = new BufferedConnection(this,
					openSocket(Constants.server_port + portOffset));
			...
			if (response == 0) {
				...
				// login block
				login.writeByte(255);
				login.writeShort(Constants.CLIENT_VERSION); //Client version
			...
			if (response == 6) {
				firstLoginMessage = Constants.getClientName() + " is being updated.";
				secondLoginMessage = "Try again in 60 secs...";
				return;
			}
			if (response == 7) {
			...
			if (entityDef.actions != null) {
				for (int i1 = 4; i1 >= 0; i1--)
					if (entityDef.actions[i1] != null
						...
						char c = '\0';
						if(!Constants.alwaysLeftClickAttack) {
	...
	private void showErrorScreen() {
		...
		if (genericLoadingError) {
			...
			g.drawString("To play " + Constants.getClientName() + " make sure you play from",
					50, 100);
		...
		if (rsAlreadyLoaded) {
			...
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of " + Constants.getClientName()
	...
	private void drawInterface(int scroll_y, int x, Widget rsInterface, int y) throws Exception {
		...
		if(rsInterface.id == 23300) {
			if(!Constants.bountyHunterInterface) {
				return;
			}
		}
		...
		if (minimapState == 2) {
			...
			if (frameMode == ScreenMode.FIXED && expCounterHover) {
				cacheSprite[23].drawSprite(
						Constants.enableOrbs && frameMode == ScreenMode.FIXED ? 0
								: frameWidth - 185,
	...
	private void drawLoginScreen(boolean flag) {
		...
		if (loginScreenState == 0) {
			...
			i = c1 / 2 - 20;
			boldText.method382(0xffff00, c / 2, "Welcome to " + Constants.CLIENT_NAME, i,
					true);
			i += 30;
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
		...
		if (loginScreenState == 3) {
			loginScreenState = 0;
			MiscUtils.launchURL(Constants.REGISTER_ACCOUNT);
		}
		loginBoxImageProducer.drawGraphics(171, super.graphics, 202);
		if (welcomeScreenRaised) {
			welcomeScreenRaised = false;
	...
	private void processMinimapActions() {
		...
		if (worldHover && Constants.enableOrbs) {
			menuActionText[1] = "World Map";
			menuActionTypes[1] = 850;
			menuActionRow = 2;
		}
		if (specialHover && Constants.enableOrbs) {
			menuActionText[1] = "Use Special Attack";
			menuActionTypes[1] = 851;
			menuActionRow = 2;
		}
		if (hpHover && Constants.enableOrbs) {
			menuActionText[1] = Constants.hpAboveHeads ? "Turn HP Above Heads on"
		...
		if (prayHover && Constants.enableOrbs) {
			menuActionText[2] =
		...
		if (runHover && Constants.enableOrbs) {
